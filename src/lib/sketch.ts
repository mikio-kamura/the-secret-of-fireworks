  ...
  constructor(
    ...
    for (let i = 0; i < this.radiusLightsNum; i++) {
      for (let j = 0; j < this.numLights + i * 3; j++) {
        ...
        let angle =
          (j / (this.numLights + i * 3)) * p5.TAU +
          p5.random(-p5.TAU / 60 - p5.TAU / 60);
        let pos = p5.createVector(
          this.center.x + p5.cos(angle) * radius,
          this.center.y + p5.sin(angle) * radius,
        );
        ...
        this.particles.push(
          new Particle(p5, this.center.x, this.center.y, pos.x, pos.y, r, g, b),
        );
      }
    }
    generatePoints(
  ...
  displayGlowingLight(): void {
    for (let particle of this.particles) {
      drawGlowingLight(
        ...
        particle.pos.x,
        particle.pos.y,
        particle.r + this.p5.random(-90, 90),
        particle.g + this.p5.random(-90, 90),
        particle.b + this.p5.random(-90, 90),
        particle.size,
      );
    }
  }
...
const generatePoints = (
  ...
  for (let i = 0; i <= segments; i++) {
    ...
    let offsetX =
      (p5.noise(x * noiseScale, y * noiseScale, 0) - 0.5) * amplitude;
    let offsetY =
      (p5.noise(x * noiseScale, y * noiseScale, 1000) - 0.5) * amplitude;
...
const drawWigglyLine = (
  ...
  for (let i = 0; i < trailPoints.length; i++) {
    ...
    let disappearTime =
      drawDuration + (i / trailPoints.length) * (lifetime - drawDuration) - 150;
